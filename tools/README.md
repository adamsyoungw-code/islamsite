...
# Импорт вопросов и тестов

`import_questions.py` добавляет в уроки открытые вопросы (поле `questions`)
и ссылки на гугл-формы с тестами (поле `testUrl`); итоговый тест цикла —
в поле `finalTestUrl` на уровне цикла. Данные зашиты в самом скрипте
(словари `QUESTIONS`, `FORMS`, `CYCLE_FINAL`) — это и есть источник.

```bash
python tools/import_questions.py
```

На странице урока показываются два блока: «Тест к уроку» (встроенная
форма — только публичные `/viewform`-ссылки) и «Вопросы к уроку» (список
без вариантов). Ссылки на редактор (`/edit`) посетителям недоступны —
валидатор помечает их предупреждением, замените на публичные.
